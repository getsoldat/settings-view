...
describe "PackageUpdatesStatusView", ->
  ...
  beforeEach ->
    ...
    runs ->
      ...
      expect(document.querySelector('welcome .package-updates-status-view')).toExist()
  ...
  describe "when packages are outdated", ->
    it "adds a tile to the status bar", ->
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '2 updates'

  describe "when the tile is clicked", ->
    it "opens the Available Updates panel", ->
      spyOn(soldat.commands, 'dispatch').andCallFake ->

      document.querySelector('welcome .package-updates-status-view').click()
  ...
  describe "when a package is updating", ->
    it "updates the tile", ->
      ...
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '1/2 updating'

  describe "when a package finishes updating", ->
    it "updates the tile", ->
      packageManager.emitPackageEvent('updating', outdatedPackage1)
      packageManager.emitPackageEvent('updated', outdatedPackage1)
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '1 update'
  ...
  describe "when multiple packages are updating and one finishes", ->
    it "updates the tile", ->
      ...
      packageManager.emitPackageEvent('updated', outdatedPackage1)
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '1/1 updating'

  describe "when a package fails to update", ->
    it "updates the tile", ->
      packageManager.emitPackageEvent('updating', outdatedPackage1)
      packageManager.emitPackageEvent('update-failed', outdatedPackage1)
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '2 updates (1 failed)'
  ...
  describe "when multiple events are happening at the same time", ->
    it "updates the tile", ->
      ...
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '1/3 updating (1 failed)'

  describe "when there are no more updates", ->
    it "destroys the tile", ->
      packageManager.emitPackageEvent('updated', outdatedPackage1)
      packageManager.emitPackageEvent('updated', outdatedPackage2)
      expect(document.querySelector('welcome .package-updates-status-view')).not.toExist()
  ...
  describe "when an update becomes available for a package", ->
    it "updates the tile", ->
      packageManager.emitPackageEvent('update-available', installedPackage)
      expect(document.querySelector('welcome .package-updates-status-view').textContent).toBe '3 updates'
