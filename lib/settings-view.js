...
export default class SettingsView {
  constructor ({uri, packageManager, snippetsProvider, activePanel} = {}) {
    ...
    this.disposables.add(soldat.commands.add(this.element, {
      ...
      <div className='settings-view pane-item' tabIndex='-1'>
        <div className='config-menu' ref='sidebar'>
          ...
          <div className='button-area'>
            <button className='btn btn-default icon icon-link-external' ref='openDotSoldat'>Open Config Folder</button>
          </div>
        </div>
  ...
  initializePanels () {
    ...
    this.refs.openDotSoldat.addEventListener('click', openDotSoldatClickHandler)
    this.disposables.add(new Disposable(() => this.refs.openDotSoldat.removeEventListener('click', openDotSoldatClickHandler)))
  ...
  getPackages () {
    ...
    try {
      const packageMetadata = require(path.join(soldat.getLoadSettings().resourcePath, 'package.json'))
      bundledPackageMetadataCache = packageMetadata ? packageMetadata._soldatPackages : null
    } catch (error) {}
