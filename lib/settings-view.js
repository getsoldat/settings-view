...
export default class SettingsView {
  ...
  render() {
    return (
      <div className="settings-view pane-item" tabIndex="-1">
        ...
        gains focus (e.g. immediately after soldat displays it), it focuses the
  ...
  initializePanels() {
    ...
    const clickHandler = event => {
      const target = event.target.closest(
        ".panels-menu li a, .panels-packages li a"
      );
      if (target) {
    ...
    const openDotSoldatClickHandler = () => {
      soldat.open({ pathsToOpen: [soldat.getConfigDirPath()] });
    };
    this.refs.openDotSoldat.addEventListener(
      "click",
      openDotSoldatClickHandler
    );
    this.disposables.add(
      new Disposable(() =>
        this.refs.openDotSoldat.removeEventListener(
          "click",
          openDotSoldatClickHandler
        )
      )
    ...
    if (process.platform === "win32" && require("soldat").WinShell != null) {
    ...
    this.addCorePanel(
      "Themes",
      "paintcan",
      () => new ThemesPanel(this, this.packageManager)
    );
    this.addCorePanel(
      "Updates",
      "cloud-download",
    ...
    this.addCorePanel(
      "Install",
      "plus",
      () => new InstallPanel(this, this.packageManager)
    );
  ...
  serialize() {
    return {
      ...
      version: 2,
      activePanel:
        this.activePanel != null ? this.activePanel : this.deferredPanel,
      uri: this.uri
    };
  }
  ...
  getPackages() {
    ...
    this.packages = soldat.packages.getLoadedPackages();

    try {
      const packageMetadata = require(path.join(
        soldat.getLoadSettings().resourcePath,
        "package.json"
      ));
      bundledPackageMetadataCache = packageMetadata
        ? packageMetadata._soldatPackages
        : null;
    } catch (error) {}

    // Include disabled packages so they can be re-enabled from the UI
    const disabledPackages = soldat.config.get("core.disabledPackages") || [];
    for (const packageName of disabledPackages) {
      var metadata;
      const packagePath = soldat.packages.resolvePackagePath(packageName);
      if (!packagePath) {
        continue;
      }
      ...
      try {
        ...
        if (
          bundledPackageMetadataCache &&
          bundledPackageMetadataCache[packageName]
        ) {
  ...
  getOrCreatePanel(name, options) {
    ...
    // this whole file.
    if (!panel) {
      let callback = this.panelCreateCallbacks
        ? this.panelCreateCallbacks[name]
        : null;
