...
export default class KeybindingsPanel {
  constructor () {
    ...
    this.disposables.add(soldat.keymaps.onDidReloadKeymap(() => { this.loadKeyBindings() }))
    this.disposables.add(soldat.keymaps.onDidUnloadKeymap(() => { this.loadKeyBindings() }))
  ...
  loadKeyBindings () {
    ...
    this.keyBindings = _.sortBy(soldat.keymaps.getKeyBindings(), 'keystrokes')
...
`
      ...
      return soldat.clipboard.write(content)
    }
  ...
  didClickOpenKeymapFile (e) {
    e.preventDefault()
    soldat.commands.dispatch(soldat.views.getView(soldat.workspace), 'application:open-your-keymap')
  }
  ...
  static determineSource(filePath) {
    ...
    if (filePath.indexOf(path.join(soldat.getLoadSettings().resourcePath, 'keymaps')) === 0) {
      return 'Core'
    } else if (filePath === soldat.keymaps.getUserKeymapPath()) {
      return 'User'
    } else {
