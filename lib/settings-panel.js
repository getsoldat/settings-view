...
import {CompositeDisposable, Disposable, TextEditor} from 'soldat'
...
export default class SettingsPanel extends CollapsibleSectionPanel {
  constructor (options = {}) {
    ...
    if (this.options.scopeName) {
      ...
      for (const name of SCOPED_SETTINGS) {
        settings[name] = soldat.config.get(name, {scope: [this.options.scopeName]})
      }
    } else {
      settings = soldat.config.get(namespace)
    }
  ...
  observe (name, callback) {
    let params = {sources: [soldat.config.getUserConfigPath()]}
    if (this.options.scopeName != null) {
      params.scope = [this.options.scopeName]
    }
    this.disposables.add(soldat.config.observe(name, params, callback))
  }
  ...
  isDefault (name) {
    ...
    let value = soldat.config.get(name, params)
    return (value == null) || (defaultValue === value)
  }

  getDefault (name) {
    let params = {excludeSources: [soldat.config.getUserConfigPath()]}
    if (this.options.scopeName != null) {
      params.scope = [this.options.scopeName]
    }

    let defaultValue = soldat.config.get(name, params)
    if (this.options.scopeName != null) {
      ...
      if (soldat.config.get(name, {excludeSources: [soldat.config.getUserConfigPath()]}) === defaultValue) {
        defaultValue = soldat.config.get(name)
      }
    }
    return defaultValue
  }
  ...
  set (name, value) {
    if (this.options.scopeName) {
      if (value === undefined) {
        soldat.config.unset(name, {scopeSelector: this.options.scopeName})
      } else {
        soldat.config.set(name, value, {scopeSelector: this.options.scopeName})
      }
    } else {
      soldat.config.set(name, value)
    }
  }
  ...
  bindEditors() {
    const disposables = Array.from(this.element.querySelectorAll('soldat-text-editor')).map((editorElement) => {
      let left
  ...
  bindTooltips () {
    const disposables = Array.from(this.element.querySelectorAll('input[id], select[id], soldat-text-editor[id]')).map((element) => {
      let defaultValue = this.valueToString(this.getDefault(element.id))
      if (defaultValue != null) {
        return soldat.tooltips.add(element, {
...
function sortSettings (namespace, settings) {
  return _.chain(settings)
    ...
    .sortBy((name) => name)
    .sortBy((name) => {
      const schema = soldat.config.getSchema(`${namespace}.${name}`)
...
function elementForSetting (namespace, name, value) {
  ...
  let schema = soldat.config.getSchema(`${namespace}.${name}`)
...
function getSettingTitle (keyPath, name) {
  ...
  const schema = soldat.config.getSchema(keyPath)
...
function elementForOptions (namespace, name, value) {
  ...
  let schema = soldat.config.getSchema(keyPath)
