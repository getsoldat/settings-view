    ...
    const notifyIfDone = () => {
      if (remainingPackagesCount === 0) {
        if (successfulUpdatesCount > 0) {
          ...
          const message = `Restart Soldat to complete the update of ${successfulUpdatesCount} ${pluralizedPackages}.`

          const buttons = [{
            text: 'Restart',
            onDidClick() { return soldat.restartApplication() }
          }]
          soldat.notifications.addSuccess(message, {dismissable: true, buttons})
        }
