...
export default class PackageUpdatesStatusView {
  initialize (statusBar, packageManager, updates) {
    ...
    const clickHandler = () => {
      soldat.commands.dispatch(soldat.views.getView(soldat.workspace), 'settings-view:check-for-package-updates')
    }
  ...
  updateTile () {
    if (this.updates.length) {
      ...
      if (this.destroyed) {
        // Priority of -99 should put us just to the left of the Squirrel icon, which displays when Soldat has updates available
      ...
      this.tooltip = soldat.tooltips.add(this.element, {title: tooltipText})
