...
import { CompositeDisposable, Disposable } from "soldat";
...
export default class PackageCard {
  constructor(pack, settingsView, packageManager, options = {}) {
    ...
    if (
      this.pack.recrueInstallSource &&
      this.pack.recrueInstallSource.type === "git"
    ) {
      if (this.pack.recrueInstallSource.sha !== this.pack.latestSha) {
    ...
    if (soldat.packages.isBundledPackage(this.pack.name)) {
      ...
      <div className="package-card col-lg-8">
        ...
        <div className="body">
          <h4 className="card-name">
            ...
            <span className="deprecation-badge highlight-warning inline-block">
              Deprecated
            </span>
          </h4>
        ...
        <div className="meta">
          <div ref="metaUserContainer" className="meta-user">
            <a ref="avatarLink" href={`https://soldat.tv/users/${owner}`}>
            ...
            <a
              ref="loginLink"
              className="author"
              href={`https://soldat.tv/users/${owner}`}
            >
              {owner}
            </a>
          </div>
  ...
  locateCompatiblePackageVersion(callback) {
    this.packageManager.loadCompatiblePackageVersion(
      this.pack.name,
      (err, pack) => {
        if (err != null) {
          console.error(err);
        ...
        if (packageVersion) {
          ...
          if (packageVersion !== this.pack.version) {
            ...
            this.refs.packageMessage.textContent = `Version ${packageVersion} is not the latest version available for this package, but it's the latest that is compatible with your version of Soldat.`;
          }
          ...
          this.refs.packageMessage.insertAdjacentText(
            "beforeend",
            `There's no version of this package that is compatible with your Soldat version. The version must satisfy ${this
              .pack.engines.soldat}.`
          );
          console.error(
            `No available version compatible with the installed Soldat version: ${soldat.getVersion()}`
          );
        }
  ...
  handleButtonEvents(options) {
    ...
    this.refs.installButton.addEventListener(
      "click",
      installButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.installButton.removeEventListener(
          "click",
          installButtonClickHandler
        );
      })
    ...
    this.refs.uninstallButton.addEventListener(
      "click",
      uninstallButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.uninstallButton.removeEventListener(
          "click",
          uninstallButtonClickHandler
        );
      })
    ...
    this.refs.installAlternativeButton.addEventListener(
      "click",
      installAlternativeButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.installAlternativeButton.removeEventListener(
          "click",
          installAlternativeButtonClickHandler
        );
      })
        ...
        const buttons = [
          {
            text: "Restart",
            onDidClick() {
              return soldat.restartApplication();
            }
          }
        ];

        soldat.notifications.addSuccess(
          `Restart Soldat to complete the update of \`${this.pack.name}\`.`,
          {
            dismissable: true,
            buttons
          }
    ...
    this.disposables.add(
      new Disposable(() => {
        this.refs.updateButton.removeEventListener(
          "click",
    ...
    const packageNameClickHandler = event => {
      ...
      shell.openExternal(`https://soldat.tv/${packageType}/${this.pack.name}`);
    };
    this.refs.packageName.addEventListener("click", packageNameClickHandler);
    this.disposables.add(
      new Disposable(() => {
        this.refs.packageName.removeEventListener(
          "click",
          packageNameClickHandler
        );
      })
    );
    ...
    const enablementButtonClickHandler = event => {
      ...
      if (this.isDisabled()) {
        soldat.packages.enablePackage(this.pack.name);
      } else {
        soldat.packages.disablePackage(this.pack.name);
      }
    };
    this.refs.enablementButton.addEventListener(
      "click",
      enablementButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.enablementButton.removeEventListener(
          "click",
          enablementButtonClickHandler
        );
      })
    ...
    const packageMessageClickHandler = event => {
      ...
      if (target) {
        event.stopPropagation();
        event.preventDefault();
        if (target.href && target.href.startsWith("soldat:")) {
          soldat.workspace.open(target.href);
        }
      }
    };
    this.refs.packageMessage.addEventListener(
      "click",
      packageMessageClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.packageMessage.removeEventListener(
          "click",
          packageMessageClickHandler
        );
      })
    ...
    this.client.package(this.pack.name, (err, data) => {
      ...
      if (!err) {
        ...
        if (
          ...
          this.refs.downloadCount.textContent = this.pack.recrueInstallSource.sha.substr(
            0,
            8
          );
        } else {
  ...
  updateInterfaceState() {
    this.refs.versionValue.textContent =
      (this.installablePack ? this.installablePack.version : null) ||
      this.pack.version;
    if (
      this.pack.recrueInstallSource &&
      this.pack.recrueInstallSource.type === "git"
  ...
  displayEnabledState() {
    ...
    this.refs.enablementButton.querySelector(".disable-text").textContent =
      "Disable";
  ...
  displayInstalledState() {
    if (this.newVersion || this.newSha) {
      ...
      if (this.newVersion) {
        ...
        this.refs.updateButton.textContent = `Update to ${this.newSha.substr(
          0,
          8
        )}`;
      }
  ...
  displayNotInstalledState() {
    ...
    const soldatVersion = this.packageManager.normalizeVersion(
      soldat.getVersion()
    );
    if (!this.packageManager.satisfiesVersion(soldatVersion, this.pack)) {
      this.hasCompatibleVersion = false;
  ...
  displayDeprecatedState() {
    ...
    if (info && info.hasDeprecations) {
      ...
      message =
        info.message != null
          ? info.message
          : `The features in \`${this.pack.name}\` have been added to core.`;
      ...
      if (isInstalled && this.packageManager.isPackageInstalled(alt)) {
        message = `\`${this.pack
          .name}\` has been replaced by \`${alt}\` which is already installed. Please uninstall this package.`;
        this.refs.settingsButton.remove();
        this.refs.enablementButton.remove();
      } else if (isInstalled) {
        message = `\`${this.pack
          .name}\` has been replaced by [\`${alt}\`](soldat://config/install/package:${alt}).`;
  ...
  handlePackageEvents() {
    let version;
    this.disposables.add(
      soldat.packages.onDidDeactivatePackage(pack => {
    ...
    this.disposables.add(
      soldat.packages.onDidActivatePackage(pack => {
        if (pack.name === this.pack.name) {
          this.updateDisabledState();
        }
      })
    ...
    this.disposables.add(
      soldat.config.onDidChange("core.disabledPackages", () => {
        this.updateDisabledState();
      })
    );
    ...
    this.subscribeToPackageEvent(
      "package-uninstalling theme-uninstalling",
      () => {
        this.updateInterfaceState();
        this.refs.enablementButton.disabled = true;
    ...
    this.subscribeToPackageEvent(
      ...
      () => {
        const loadedPack = soldat.packages.getLoadedPackage(this.pack.name);
        const version =
          loadedPack && loadedPack.metadata
            ? loadedPack.metadata.version
            : null;
        if (version) {
    ...
    this.subscribeToPackageEvent("package-updated theme-updated", () => {
      ...
      if (metadata && metadata.recrueInstallSource) {
        this.pack.recrueInstallSource = metadata.recrueInstallSource;
      }
    ...
    this.subscribeToPackageEvent(
      "package-update-failed theme-update-failed",
      () => {
        this.refs.updateButton.disabled = false;
        this.refs.updateButton.classList.remove("is-installing");
        this.updateInterfaceState();
    ...
    this.subscribeToPackageEvent(
      "package-installed-alternative package-install-alternative-failed",
      () => {
  ...
  isDisabled() {
    return soldat.packages.isPackageDisabled(this.pack.name);
  }

  isDeprecated(version) {
    const packVersion = version != null ? version : this.pack.version;
    return soldat.packages.isDeprecatedPackage(this.pack.name, packVersion);
  }

  getDeprecatedPackageMetadata() {
    return soldat.packages.getDeprecatedPackageMetadata(this.pack.name);
  }
  ...
  */
  ...
  update() {
    ...
    const version = this.newVersion
      ? `v${this.newVersion}`
      : `#${this.newSha.substr(0, 8)}`;
    return new Promise((resolve, reject) => {
      this.packageManager.update(pack, this.newVersion, error => {
        if (error != null) {
          soldat.assert(false, "Package update failed", assertionError => {
  ...
  installAlternative() {
    ...
    const loadedPack = soldat.packages.getLoadedPackage(
      metadata ? metadata.alternative : null
    );
    if (
      !metadata ||
      !metadata.hasAlternative ||
      metadata.alternative === "core" ||
      loadedPack
    ) {
      return;
    }

    this.packageManager.installAlternative(
      this.pack,
      metadata.alternative,
      (error, { pack, alternative }) => {
        if (error != null) {
          console.error(
            `Installing alternative \`${alternative}\` ${this.type} for ${this
              .pack.name} failed`,
            error.stack != null ? error.stack : error,
            error.stderr
          );
