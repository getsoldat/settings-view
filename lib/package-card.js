...
export default class PackageCard {
    ...
    this.packageManager.loadCompatiblePackageVersion(this.pack.name, (err, pack) => {
      ...
      if (packageVersion) {
        ...
        this.refs.packageMessage.insertAdjacentText(
          'beforeend',
          `There's no version of this package that is compatible with your Soldat version. The version must satisfy ${this.pack.engines.soldat}.`
        )
        console.error(`No available version compatible with the installed Soldat version: ${soldat.getVersion()}`)
      }
  ...
  handleButtonEvents (options) {
    ...
    const updateButtonClickHandler = (event) => {
      ...
      this.update().then(() => {
        const buttons = [{
          text: 'Restart',
          onDidClick () { return soldat.restartApplication() }
        }]

        soldat.notifications.addSuccess(`Restart Soldat to complete the update of \`${this.pack.name}\`.`, {
  ...
  displayNotInstalledState () {
    ...
    const soldatVersion = this.packageManager.normalizeVersion(soldat.getVersion())
  ...
  displayDeprecatedState () {
    ...
    if (info && info.hasDeprecations) {
      ...
      if (isInstalled && this.packageManager.isPackageInstalled(alt)) {
        ...
        message = `\`${this.pack.name}\` has been replaced by [\`${alt}\`](soldat://config/install/package:${alt}).`
  ...
  handlePackageEvents () {
    let version
    this.disposables.add(soldat.packages.onDidDeactivatePackage((pack) => {
      if (pack.name === this.pack.name) {
        this.updateDisabledState()
      }
    }))

    this.disposables.add(soldat.packages.onDidActivatePackage((pack) => {
      if (pack.name === this.pack.name) {
        this.updateDisabledState()
      }
    }))

    this.disposables.add(soldat.config.onDidChange('core.disabledPackages', () => {
      this.updateDisabledState()
    }))
  ...
  */
  ...
  installAlternative () {
    ...
    const loadedPack = soldat.packages.getLoadedPackage(metadata ? metadata.alternative : null)
