...
import {CompositeDisposable, TextEditor} from 'soldat'
...
export default class ThemesPanel extends CollapsibleSectionPanel {
  ...
  constructor (settingsView, packageManager) {
    ...
    this.disposables.add(soldat.themes.onDidChangeActiveThemes(() => this.updateActiveThemes()))
    this.disposables.add(soldat.tooltips.add(this.refs.activeUiThemeSettings, {title: 'Settings'}))
    this.disposables.add(soldat.tooltips.add(this.refs.activeSyntaxThemeSettings, {title: 'Settings'}))
        ...
        <div className='section packages themes-panel'>
          <div className='section-container'>
            ...
            <div className='text native-key-bindings' tabIndex='-1'>
              <span className='icon icon-question'>You can also style Soldat by editing </span>
  ...
  filterThemes (packages) {
    ...
    for (let pack of packages.core) {
      if (pack.repository == null) {
        pack.repository = `https://github.com/soldat/${pack.name}`
      }
    }
  ...
  populateThemeMenus () {
    ...
    const availableThemes = _.sortBy(soldat.themes.getLoadedThemes(), 'name')
  ...
  getActiveUiTheme () {
    for (let {name, metadata} of soldat.themes.getActiveThemes()) {
  ...
  updateThemeConfig () {
    ...
    if (themes.length > 0) {
      soldat.config.set("core.themes", themes)
    }
  }
  ...
  didClickOpenUserStyleSheet (e) {
    e.preventDefault()
    soldat.commands.dispatch(soldat.views.getView(soldat.workspace), 'application:open-your-stylesheet')
  }
  ...
  didClickActiveUiThemeSettings (event) {
    ...
    const theme = soldat.themes.getActiveThemes().find((theme) => theme.metadata.theme === 'ui')
  ...
  didClickActiveSyntaxThemeSettings (event) {
    event.stopPropagation()
    const theme = soldat.themes.getActiveThemes().find((theme) => theme.metadata.theme === 'syntax')
