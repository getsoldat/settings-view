...
export default class PackageDetailView {
  constructor(pack, settingsView, packageManager) {
    ...
    this.disposables.add(
      soldat.commands.add(this.element, {
        "core:move-up": () => {
          this.scrollUp();
        },
        "core:move-down": () => {
    ...
    this.disposables.add(
      new Disposable(() => {
        this.refs.packageRepo.removeEventListener(
          "click",
          packageRepoClickHandler
        );
      })
    );
    ...
    this.disposables.add(
      new Disposable(() => {
        this.refs.changelogButton.removeEventListener(
          "click",
          issueButtonClickHandler
        );
      })
    );
    ...
    this.refs.changelogButton.addEventListener(
      "click",
      changelogButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.changelogButton.removeEventListener(
    ...
    this.refs.licenseButton.addEventListener(
      "click",
      licenseButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.licenseButton.removeEventListener(
          "click",
          licenseButtonClickHandler
        );
      })
    ...
    const openButtonClickHandler = event => {
      ...
      if (fs.existsSync(this.pack.path)) {
        soldat.open({ pathsToOpen: [this.pack.path] });
      }
    };
    ...
    const learnMoreButtonClickHandler = event => {
      event.preventDefault();
      shell.openExternal(`https://soldat.tv/packages/${this.pack.name}`);
    };
    this.refs.learnMoreButton.addEventListener(
      "click",
      learnMoreButtonClickHandler
    );
    this.disposables.add(
      new Disposable(() => {
        this.refs.learnMoreButton.removeEventListener(
          "click",
          learnMoreButtonClickHandler
        );
      })
    ...
    this.disposables.add(
      new Disposable(() => {
        this.refs.breadcrumb.removeEventListener(
          "click",
  ...
  completeInitialization() {
    if (this.refs.packageCard) {
      ...
      this.refs.packageCardParent.replaceChild(
        this.packageCard.element,
        this.refs.loadingMessage
      );
    }
  ...
  loadPackage() {
    const loadedPackage = soldat.packages.getLoadedPackage(this.pack.name);
    if (loadedPackage) {
  ...
  activateConfig() {
    ...
    if (
      soldat.packages.isPackageLoaded(this.pack.name) &&
      !soldat.packages.isPackageActive(this.pack.name)
    ) {
  ...
  populate() {
    this.refs.title.textContent = `${_.undasherize(
      _.uncamelcase(this.pack.name)
    )}`;
  ...
  updateInstalledState() {
    ...
    if (soldat.packages.isPackageLoaded(this.pack.name)) {
      if (!soldat.packages.isPackageDisabled(this.pack.name)) {
        ...
        this.refs.startupTime.innerHTML = `This ${this
          .type} added <span class='highlight'>${this.getStartupTime()}ms</span> to startup time.`;
    ...
    const sourceIsAvailable =
      ...
      !soldat.packages.isBundledPackage(this.pack.name);
  ...
  subscribeToPackageManager() {
    this.disposables.add(
      this.packageManager.on(
        "theme-installed package-installed",
        ({ pack }) => {
    ...
    this.disposables.add(
      this.packageManager.on(
        "theme-uninstalled package-uninstalled",
        ({ pack }) => {
          if (this.pack.name === pack.name) {
  ...
  openMarkdownFile(path) {
    if (soldat.packages.isPackageActive("markdown-preview")) {
      soldat.workspace.open(encodeURI(`markdown-preview://${path}`));
    } else {
      soldat.workspace.open(path);
    }
  }
  ...
  updateFileButtons() {
    ...
    const packagePath =
      this.pack.path != null
        ? this.pack.path
        : soldat.packages.resolvePackagePath(this.pack.name);
  ...
  getStartupTime() {
    ...
    const activateTime =
      this.pack.activateTime != null ? this.pack.activateTime : 0;
