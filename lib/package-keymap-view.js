...
import {Disposable, CompositeDisposable} from 'soldat'
...
export default class PackageKeymapView {
  constructor (pack) {
    ...
    const packagesWithKeymapsDisabled = soldat.config.get('core.packagesWithKeymapsDisabled') || []
    ...
    const changeHandler = (event) => {
      ...
      if (value) {
        soldat.config.removeAtKeyPath('core.packagesWithKeymapsDisabled', this.namespace)
      } else {
        soldat.config.pushAtKeyPath('core.packagesWithKeymapsDisabled', this.namespace)
      }
    ...
    for (let [packageKeymapsPath, map] of soldat.packages.getLoadedPackage(this.namespace).keymaps) {
      if (map.length > 0) {
        hasKeymaps = true
        break
      }
  ...
  updateKeyBindingView () {
    ...
    if (soldat.keymaps.build) {
      for (const [keymapPath, keymap] of soldat.packages.getLoadedPackage(this.namespace).keymaps) {
        keyBindings.push(...soldat.keymaps.build(this.namespace, keymap, 0, false))
      }
    } else {
      // Backwards compatibility for Soldat <= 1.19
      for (const keyBinding of soldat.keymaps.getKeyBindings()) {
  ...
  writeKeyBindingToClipboard ({selector, keystrokes, command}) {
    let content
    const keymapExtension = path.extname(soldat.keymaps.getUserKeymapPath())
...
`
    ...
    soldat.clipboard.write(content)
  }
}
